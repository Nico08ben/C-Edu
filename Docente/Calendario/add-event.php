...
$response = ["success" => false, "message" => "Error desconocido al procesar el evento."];
...
if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
 ...
 if ($data === null && empty($input_data_raw)) {
...
$categoria_evento = $data["categoria_evento"] ?? "Otro"; // Stays, but not sent from current JS
$enlace_recurso = $data["enlace_recurso"] ?? null;   // Stays, but not sent from current JS
$color_evento = $data["color"] ?? null; // <<< --- ADD THIS LINE to get color
...
try {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO evento (fecha_evento, hora_evento, tipo_evento, asignacion_evento, categoria_evento, id_responsable, enlace_recurso, color_evento)
         VALUES (:fecha, :hora, :titulo, :descripcion, :categoria, :id_responsable, :enlace, :color_evento)"
    );
    ...
    $stmt->bindParam(':color_evento', $color_evento); // <<< --- ADD THIS BINDING
    ...
    if ($stmt->execute()) {
        ...
        $response["db_error_code"] = $errorInfo[1] ?? null;
    }
