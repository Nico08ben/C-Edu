...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const notificacionesContador = document.getElementById('notificacionesContador');
    ...
    const idUsuarioActual = typeof ID_USUARIO_LOGUEADO !== 'undefined' ? ID_USUARIO_LOGUEADO : null;

    if (!idUsuarioActual) {
        console.warn('Módulo de notificaciones: ID de usuario no encontrado. Las notificaciones no funcionarán.');
        if (notificacionesIcono) notificacionesIcono.style.display = 'none'; // Ocultar si no hay usuario
        return;
    }

    const API_URL_OBTENER = 'api/obtener_notificaciones.php'; // Ajusta la ruta si es necesario
    const API_URL_MARCAR_LEIDA = 'api/marcar_leida.php';
    const API_URL_MARCAR_TODAS = 'api/marcar_todas_leidas.php';
    const POLLING_INTERVAL = 30000; // 30 segundos

    function formatearFecha(fechaISO) {
        const fecha = new Date(fechaISO);
        return fecha.toLocaleString('es-CO', { // Ajusta a tu localidad preferida
    ...
    async function cargarNotificaciones() {
        ...
        try {
            const response = await fetch(`${API_URL_OBTENER}?id_usuario=${idUsuarioActual}&limite=10`); // Pide las últimas 10
            if (!response.ok) {
                console.error('Error en la respuesta del servidor al cargar notificaciones:', response.status);
                listaNotificaciones.innerHTML = '<li class="sin-notificaciones">Error al cargar.</li>';
                return;
            }
            ...
            if (data.notificaciones && data.notificaciones.length === 0) {
                listaNotificaciones.innerHTML = '<li class="sin-notificaciones">No tienes notificaciones.</li>';
            } else if (data.notificaciones) {
                data.notificaciones.forEach(notif => {
                    const li = document.createElement('li');
                    li.dataset.id = notif.id_notificacion;
                    li.classList.add(notif.estado_notificacion === 'no leída' ? 'no-leida' : 'leida');
                    
                    let contenidoHTML = `<span class="mensaje">${notif.mensaje || 'Notificación sin mensaje.'}</span>`;
                    if (notif.tipo_notificacion) {
                        contenidoHTML += `<span class="tipo">Tipo: ${notif.tipo_notificacion.replace(/_/g, ' ')}</span>`;
                    }
                    contenidoHTML += `<span class="fecha">${formatearFecha(notif.fecha_notificacion)}</span>`;
            ...
            listaNotificaciones.innerHTML = '<li class="sin-notificaciones">Error al cargar. Intenta de nuevo.</li>';
        }
    }
        ...
        if (notificacion.estado_notificacion === 'no leída') {
            try {
                const response = await fetch(API_URL_MARCAR_LEIDA, {
                    ...
                    body: JSON.stringify({ id_notificacion: notificacion.id_notificacion }) // id_usuario se toma de la sesión en PHP
                });
    ...
    if (notificacionesIcono) {
        notificacionesIcono.addEventListener('click', (event) => {
            ...
            notificacionesDropdown.style.display = display === 'none' ? 'block' : 'none';
    ...
    document.addEventListener('click', function(event) {
        if (notificacionesDropdown && notificacionesIcono) {
            const isClickInside = notificacionesIcono.contains(event.target) || notificacionesDropdown.contains(event.target);
            if (!isClickInside && window.getComputedStyle(notificacionesDropdown).display === 'block') {
                notificacionesDropdown.style.display = 'none';
            }
        }
    });
