...
// ...
// En lugar de los echo:
if ($conn->query($sql) === TRUE) {
    $_SESSION['user_management_message'] = "Usuario actualizado correctamente.";
    $_SESSION['user_management_message_type'] = "success";
} else {
    $_SESSION['user_management_message'] = "Error al actualizar usuario: " . $conn->error;
    $_SESSION['user_management_message_type'] = "error";
    error_log("Error SQL update: " . $conn->error); // Mantener el error_log
}

} else { // Si no es POST
   $_SESSION['user_management_message'] = "Método no permitido para actualizar.";
   $_SESSION['user_management_message_type'] = "error";
}
